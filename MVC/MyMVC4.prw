...
/*/{Protheus.doc} User Function MyMVC_4
  Exemplo de uso de Eventos da View e Browse com coluna de marcaùùo
  @type  Function
...
User Function MyMVC_4()
  Local cAlias  := 'ZZC'
  Local cTitle  := 'Cursos de Programaùùo'
  Local oMark   := FWMarkBrowse():New() //? Instanciando a Classe
  
  //? Adiconando botùes (nùo ù necessario ter MenuDef)
  ...
  //? Definiùùo da tabela a ser utilizada
  oMark:SetAlias(cAlias)

  //? Define o tùtulo do browse de marcaùùo
  oMark:SetDescription(cTitle)

  //? Define o campo que serù utilizado para a marcaùùo
  oMark:SetFieldMark('ZZC_MARC')

  //? Desabilita os detalhes
  oMark:DisableDetails()

  //? Desabilita o botùo de impressùo
  oMark:DisableReport()
...
Static Function ViewDef()
  ...
  //? Com o valor .F., ao clicar em "Confirmar" na inclusùo, a tela de cadatro serù limpa e permanecerù aberta. Se estiver como .T. a tela de cadastro fecha apùs a confirmaùùo do cadastro.
  oView:SetCloseOnOk({|| .F.}) 

  //? Executa a aùùo do bloco de cùdigo apùs pressionar o botùo "Confirmar"
  oView:SetAfterOkButton({|| FwAlertSuccess('Tudo certo!', 'SetAfterOkButton')})

  //? Verifica se a tela pode ser aberta ou nùo.
  oView:SetViewCanActivate({|| MsgYesNo('Ativar a tela?', 'SetViewCanActivate')})

  //? Esse mùtodo ù executado apùs a ativaùùo da View
  oView:SetAfterViewActivate({|| FwAlertInfo('', 'Tela Ativada!')})
Return oView

//? Funùùo para Marcar todos os registros 
User Function Marca()
  DbSelectArea('ZZC')
  ...
  while ZZC->(!EOF())
    if !oMark:IsMark() //? Se nùo estiver marcado
  ...
  //? Executa a atualizaùùo das informaùùes no Browse
  oMark:Refresh(.T.) //? .T. Indica que deverù ser posicionado no primeiro registro do Browse
Return

//? Funùùo para Desmarcar todos os registros 
...
//? Funùùo para inverter marcaùùo de todos os registros 
...
//? Funùùo para deletar registros marcados
User Function Deleta()
  if MsgYesNo('Confirma a exclusùo dos cursos selecionados?')
    DbSelectArea('ZZC')
    
    ZZC->(DbGotop())
    
    while ZZC->(!EOF())
      if oMark:IsMark()   //? Verifica se o registro estù marcado
          RecLock('ZZC', .F.)
