...
/*/{Protheus.doc} User Function MyBrowse
  Exemplo de usoValidaùùes utilizando rotina em MVC
  @type  Function
...
Static Function ModelDef()
  Local bModVldActiv := {|oMdl| FwAlertInfo('Vamos ao cadastro!', 'Cadastro de Curso'), .T.} //? Bloco de cùdigo que serù chamado na Prù-validaùùo (MODELVLDACTIVE)
  Local bModelPre    := {|oMdl| ValidPre(oMdl)} //? Bloco de cùdigo que serù chamado na Prù-validaùùo (MODELPRE)
  Local bModelPos    := {|oMdl| ValidPos(oMdl)} //? Bloco de cùdigo que serù chamado na Pùs-validaùùo (MODELPOS)
  Local bModelCancel := {|oMdl| Cancel(oMdl)}   //? Bloco de cùdigo que serù chamado na Pùs-validaùùo (MODELCANCEL)
  ...
  //? Definindo que o campo ZZB_COD nùo pode se repetir na grade.
  oModel:GetModel('ZZBDETAIL'):SetUniqueLine({'ZZB_COD'})

  //? Adicionando validaùùo na inicializaùùo do modelo (MODELVLDACTIVE)
  oModel:SetVldActivate(bModVldActiv)
Return oModel

Static Function ViewDef()
  Local oModel   := FWLoadModel('MYMVC_3') 
  Local oStruZZC := FWFormStruct(2, 'ZZC' ) //? Pegando a estrutura da 1ù tabela (Fields)
  Local oStruZZB := FWFormStruct(2, 'ZZB' ) //? Pegando a estrutura da 2ù tabela (Grid)
  Local oView
  ...
  oView:SetOwnerView('VIEW_ZZC', 'CURSO') //* VIEW_ZZC ù o nome que foi definido na linha 73
  oView:SetOwnerView('VIEW_ZZB', 'ALUNOS') //* VIEW_ZZB ù o nome que foi definido na linha 76
  oView:EnableTitleView('VIEW_ZZB', 'Alunos')
Return oView

//? Funùùo chamada pela prù validaùùo do Model
Static Function ValidPre(oMdl)
  Local nOperation  := oMdl:GetOperation()
  Local lAutorizado := .T.

  if nOperation == 4 //? Alteraùùo
...
Static Function ValidPos(oMdl)
  Local nOperation  := oMdl:GetOperation()
  Local cConteudo   := oMdl:GetValue('ZZCMASTER', 'ZZC_NOME') //? Pega o conteùdo do campo "NOME" da tabela "ZZC" (CURSOS)
  Local lPreenchido := !Empty(cConteudo) //? Verifica se o campo estù vazio.
  Local lTudoOk     := .T.
  
  if nOperation == 5 //! Exclusùo
    Help(NIL, NIL, 'Nùo autorizado', NIL, 'Vocù nùo tem autorizaùùo para fazer exclusùes!', 1, 0, NIL, NIL, NIL, NIL, NIL, {'Contate o administrador!'})
    lTudoOk := .F.
  elseif !lPreenchido
    Help(NIL, NIL, 'Nùo ù possùvel gravar o curso', NIL, 'O <b>Nome do Curso</b> nùo foi preenchido!', 1, 0, NIL, NIL, NIL, NIL, NIL, {'Preencha o <b>Nome do Curso</b>!'})
    lTudoOk := .F.
  endif
Return lTudoOk

//? Funùùo chamada quando o usuùrio cancela a operaùùo 
Static Function Cancel(oMdl)
  Local lCancela := .F.
  Local cMsg     := 'Deseja cancelar a operaùùo?'
  
  FwFormCancel(oMdl)
  lCancela := MsgYesNo(cMsg, 'Cancelar?')
Return lCancela

//? Pùs Validaùùo dos Fields
Static Function ValidField(oFld)
  Local lTudoOk := .T.
  Local cCod    := oFld:GetValue('ZZC_COD') 

  if cCod == '000000'
    Help(NIL, NIL, 'Operaùùo nùo permitida', NIL, 'O cùdigo do curso nùo pode ser <b>000000</b>!', 1, 0, NIL, NIL, NIL, NIL, NIL, {'Preencha o <b>Cùdigo do curso</b> corretamente!'})
    lTudoOk := .F.
  endif
Return lTudoOk
...
Static Function ValidLine(oGrid, nLine, cAction, cFieldId, xValue, xCurValue)
  ...
  if cFieldId == 'ZZB_COD' .AND. cAction == 'SETVALUE' .AND. xValue == xCurValue
    Help(NIL, NIL, 'Nùo tem por que fazer isso', NIL, 'Vocù digitou o mesmo cùdigo!', 1, 0, NIL, NIL, NIL, NIL, NIL, {'Digite outra coisa!'})
    lValido := .F.
  endif
Return lValido
...
Static Function ValidPosLine(oGrid, nLine)
  ...
  if cAluno == 'MURIEL'
    Help(NIL, NIL, 'Aluno proibido', NIL, 'O aluno Muriel estù proibido de estudar aqui!', 1, 0, NIL, NIL, NIL, NIL, NIL, {'Selecione outro aluno!'})
    lValido := .F.
  endif
Return lValido
